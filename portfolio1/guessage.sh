...
if ! [ -f "userguess.txt" ]; then  #if a file does not already exist
    touch userguess.txt            #create an empty file to hold guesses
else
    echo "" >| userguess.txt       #else if the file exists overwrite with an empty file        
fi
...
while [ "$guess" != "$age" ]; do                                        
    ...
    #else if the guess is not between 20 and 70 tell the user the guess is out of range
    elif [[ "$guess" -le 20 ]] || [[ "$guess" -ge 70 ]]; then                        
        echo -e "\033[31mYour guess is not within range.\033[0m Try again.\n"            
    
    #otherwise if input is a valid guess
    else                                                      
        if ! [[ -s "userguess.txt" ]]; then     #if the 'userguess' file is empty
            echo $guess >> userguess.txt        #append guess to the file
            ...
            if [ $? -eq 0 ]; then                           #if search returns true 
                ...
                "tried that number.\033[0m Try again.\n"    #tell the user to try again  
