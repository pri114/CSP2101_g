...
# Otherwise if the file exists
    ...
    # check it's a text file and search the file contents to ensure it's in the right format.
    if [[ $selfile == *.txt ]]; then 
        ...
        if [ $? -eq 0 ]; then                  # If true the text file can be formatted.
            ...
            # Find every instance of the word 'Rec' and add the word 'Name: ' before it
            # Locate the first comma in each line and substitute with a tab and a heading
                # i.e. the first comma in each line will be replaced with a tab and the word 'Height:'
                # the following comma (now the first) will be replaced with a tab and the word 'Width:' 
                # and so forth until all four commas in each line have been replaced.
                sed -e '1d'\
                -e 's/Rec/Name: &/g'\
                -e 's/,/\tHeight: /'\
                -e 's/,/\tWidth: /'\
                -e 's/,/\tArea: /'\
                -e 's/,/\tColour: /' $selfile > $formfile 
                # Write the formatted output to the newly created file stored in the variable 'formfile'.

        # If the file is not in the right format let the user know and exit.
        else    
